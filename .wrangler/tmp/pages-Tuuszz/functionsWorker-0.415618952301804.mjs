...
// [[path]].js
var app = new Hono2();
var ensurePage = /* @__PURE__ */ __name(async (supabase, pageName) => {
  const { error } = await supabase.from("pages").upsert(
    { name: pageName, created_at: (/* @__PURE__ */ new Date()).toISOString() },
...
app.get("/entries", async (c) => {
  ...
  const { data, error } = await supabase.from("entries").select("*").eq("page_name", pageName).order("created_at", { ascending: false });
  if (error) {
...
// ../.wrangler/tmp/pages-Tuuszz/functionsRoutes-0.4341982111119631.mjs
var routes = [
  {
    routePath: "/:path*",
    mountPath: "/",
...
// ../.wrangler/tmp/bundle-koaxm6/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-koaxm6/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.415618952301804.mjs.map
